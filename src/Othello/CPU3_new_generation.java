...
public class CPU3_new_generation extends Player{


	public CPU3_new_generation(Stone getColor, View getBoard, int getLimit){
		super(getColor);
	...
	private int evaluate(Board board) {
		...
		//左上
		if(board.data[0][0] == 0){
			if(board.data[1][1] == this.stoneColor.color){
				eval = eval - Xmove_w;
			} else if(board.data[1][1] == this.stoneColor.color * -1){
				eval = eval + Xmove_w;
			}
		}
		//左下
		if(board.data[0][7] == 0){
			if(board.data[1][6] == this.stoneColor.color){
				eval = eval - Xmove_w;
			} else if(board.data[1][6] == this.stoneColor.color * -1){
				eval = eval + Xmove_w;
			}
		}
		//右上
		if(board.data[7][0] == 0){
			if(board.data[6][1] == this.stoneColor.color){
				eval = eval - Xmove_w;
			} else if(board.data[6][1] == this.stoneColor.color * -1){
				eval = eval + Xmove_w;
			}
		}
		//右下
		if(board.data[7][7] == 0){
			if(board.data[6][6] == this.stoneColor.color){
				eval = eval - Xmove_w;
			} else if(board.data[6][6] == this.stoneColor.color * -1){
				eval = eval + Xmove_w;
			}
		}
